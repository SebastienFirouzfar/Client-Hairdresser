...
const renderCalendar = () => {
    ...
    for (let i = 1; i <= lastDay; i++) {
        ...
        if (i === new Date().getDate() && date.getMonth() === new Date().getMonth() && date.getFullYear() === new Date().getFullYear()) {
            //days += `<a href="/buttons"><div class="today">${i}</div></a>`;
            days +=
                `<form action="#" method="post">
<a href="/buttons" onclick="parentNode.submit();"><div class="today">${i}</div></a>
<input type="hidden" name="fulldate" value="${i}/${date.getMonth()+1}/${date.getFullYear()}">
</form>`
            ...
            if (is_weekend(t) === true) {
                if (i < new Date().getDate() && date.getMonth() === new Date().getMonth() && date.getFullYear() === new Date().getFullYear()) {
                    ...
                    //days += `<a href="/buttons"><div class="weekend">${i}</div></a>`;
                    days +=
                    `<form action="#" method="post">
    <a href="/buttons" onclick="parentNode.submit();"><div class="weekend">${i}</div></a>
    <input type="hidden" name="fulldate" value="${i}/${date.getMonth()+1}/${date.getFullYear()}">
    </form>`
                }
                ...
                if (i < new Date().getDate() && date.getMonth() === new Date().getMonth() && date.getFullYear() === new Date().getFullYear()) {
                    ...
                    //days += `<a href="/buttons"><div class="current">${i}</div></a>`;
                    days +=
                    `<form action="#" method="post">
    <a href="/buttons" onclick="parentNode.submit();"><div class="current">${i}</div></a>
    <input type="hidden" name="fulldate" value="${i}/${date.getMonth()+1}/${date.getFullYear()}">
    </form>`
